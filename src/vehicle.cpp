...
Vehicle Vehicle:: choose_next_state(map<int,vector<Vehicle>>predictions)
{
    ...
    for (vector<string>::iterator it = states.begin(); it != states.end(); ++it) {
        ...
        if (trajectory.size() != 0) {
        	// cout<<"printing the size of trajectory"<<" "<<trajectory.size()<<endl;
            cost = calculate_cost(*this, predictions, trajectory);
            cout<<cost<<endl;
            // cout<<"printing out costs"<<endl;
            // cout<<cost<<" "<<trajectory[0].lane<<" "<<trajectory[0].state<<endl;
...
vector<double> Vehicle::get_kinematics(map<int, vector<Vehicle>> predictions, int lane) {
    ...
    */
    ...
    // double s = this->s +this->v + 0.5 * this->a;

    if (get_vehicle_ahead(predictions, lane, vehicle_ahead)) {

    	// cout<<"YES THE VEHICLE IS AHEAD"<<endl;

        if (get_vehicle_behind(predictions, lane, vehicle_behind)) {
            new_velocity = vehicle_ahead.v; //must travel at the speed of traffic, regardless of preferred buffer
            // cout<<"yes I am here in"<<endl;
        } else {
        	double s = this->s + this->v;
            double max_velocity_in_front = (vehicle_ahead.s - s - this->preferred_buffer) + vehicle_ahead.v - 0.5 * (this->a);
            // cout<<"I am here in the loop"<<" "<<vehicle_ahead.v<<" "<<max_velocity_in_front<<" "<<vehicle_ahead.s - this->s - this->preferred_buffer;
            new_velocity = min(min(max_velocity_in_front, max_velocity_accel_limit), this->target_speed);
            // new_velocity= vehicle_ahead.v;
            // cout<<new_velocity<<endl;
        }
    } else {
    ...
    // cout<<"The new acceleration is"<<" "<<new_accel<<endl;

    // cout<<"The new acceleration is"<<" "<<new_accel<<endl;
    new_position = this->s + new_velocity + new_accel / 2.0;
    // cout<<"The new velocity is"<<" "<<new_velocity<<endl;
    // cout<<"the new position is"<<" "<<new_position<<endl;
...
vector<Vehicle> Vehicle::lane_change_trajectory(string state, map<int, vector<Vehicle>> predictions) {
    ...
    */
    ...
    //     if ((next_lane_vehicle.s + 30 >=this->s || next_lane_vehicle.s -30<=this.s)  && next_lane_vehicle.lane == new_lane) {
